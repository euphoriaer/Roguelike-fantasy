Shader "Custom/DistanceMaskLit"
{
    Properties
    {
        ...
        [HDR] _EmissionColor("自发光Color", Color) = (0,0,0)
        [Toggle(_EMISSION)]_Emission("自发光", int) = 0
        _EmissionMap("Emission", 2D) = "white" {}
        [Header(MapMask)]
        _MapStartX("地图X偏移", float) =0
        _MapStartZ("地图Z偏移", float) =0
        _Mask("Mask", 2D) = "white" {}
        [HDR]_MaskColor ("MaskColor", Color) = (0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                half4 c = LitPassFragment(input);
                real  _MapSize = abs(_MapStartX) + abs(_MapStartZ);
                half3 pos =  input.positionWS;
                pos.x += abs(_MapStartX);
                pos.z += abs(_MapStartZ);
                half2 uv = half2(0,0);
                uv.x = saturate(pos.x / _MapSize);
                uv.y = saturate(pos.z / _MapSize);
                half4 mask = tex2D(_Mask, uv);
                c.rgb =  lerp(c.rgb, c.rgb *_MaskColor, mask.r); 
                return c;
            }
            ENDHLSL
        }
